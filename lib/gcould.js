...
export class GCloudUploader {
    ...
    async uploadInChunks() {
        ...
        try {
            ...
            let startByte = await this.getUploadProgress();
            await this.uploadChunksFromByte(startByte);
        } catch (error) {
    ...
    async getUploadProgress() {
        try {
            // Query GCS to check upload progress
            const response = await this.fetchRetry(this.url, {
                method: "PUT",
                headers: { "Content-Range": `bytes */${this.file.size}` }
            }, this.delayBetweenRetryCalls, this.retryChunkUploadLimit);

            if (response.status === 308) {
                // Upload incomplete, check range header
    ...
    async uploadChunksFromByte(startByte) {
        let offset = startByte;
        this.bytesUploaded = startByte;
